...
/*---------------------------------------------------------------------------*/
	...
	{
		gotoxy(width+3,0); //out window of the game from right @height=0
		textcolour(10);
		cout<<"MADE BY=Yasmin,NadaE,NadaF,Naiera";
		textcolour(6);
		gotoxy(width+3,2); //out window of the game from right @height=2
		...
		if(choice==2) //in case of 2 players will be executed
		{
			...
			gotoxy(width+17,6); //out window of the game from right @ width= 117 and @height=2
			cout<<"  ";
			gotoxy(width+19,6);
			cout<<fruitcounter;
		}
...
/*---------------------------------------------------------------------------*/
	void welcome() //function to display welcome window
	{
        ...
        box(25,10,75,30); //sent values to box function
...
/*---------------------------------------------------------------------------*/
		void checkup2() // test cases for second snake in second player mode
	{
		...
		h=head2->next;
    while(h!=NULL) //player1 lose when snake1 hit snake2  (enhancement)
		{
			if(x==h->nx&&y==h->ny) //when head = one of the nodes of the snake
			{
				gameover1=1;
		...
		h=head->next;
    while(h!=NULL) //player2 lose when snake2 hit snake1  (enhancement)
		{
			if(x2==h->nx&&y2==h->ny) //when head = one of the nodes of the snake
			{
				gameover2=1;
				break;
			}
